...
class Beam(object):
    ...
    def advance(self, wordLk, attnOut, topicAttnOut, mixGateOut):
        ...
        self.topicAttn.append(topicAttnOut.index_select(0, prevK))
        self.mixGate.append(mixGateOut.index_select(0, prevK))
    ...
    def getHyp(self, k):
        hyp, attn, topicAttn, mixGate = [], [], [], []
        # print(len(self.prevKs), len(self.nextYs), len(self.attn))
        for j in range(len(self.prevKs) - 1, -1, -1):
            hyp.append(self.nextYs[j + 1][k])
            attn.append(self.attn[j][k])
            topicAttn.append(self.topicAttn[j][k])
            mixGate.append(self.mixGate[j][k])
            k = self.prevKs[j][k]

        return hyp[::-1], torch.stack(attn[::-1]), torch.stack(topicAttn[::-1]), torch.stack(mixGate[::-1])
